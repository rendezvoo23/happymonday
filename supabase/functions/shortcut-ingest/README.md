...
Supabase service-role key, or OpenAI key. It stores only a revocable credential
...
```text
OPENAI_API_KEY=<OpenAI project key restricted to Chat Completions requests>
...
SHORTCUT_AI_MODE=openai
SHORTCUT_AI_ENABLED=true
AUTH_ALLOWED_ORIGIN=https://your-mini-app.example
```

Set `SHORTCUT_AI_ENABLED=false` to stop all provider calls immediately without
revoking user tokens. This is the emergency budget kill switch.
...
13. Enable OpenAI mode and repeat dry-run tests before allowing real inserts.
...
When the input has no explicit date or time, the server clock is authoritative.
The model timestamp is used only for phrases such as `вчера` or `в 14:30`.

Production limits are enforced atomically in PostgreSQL: 15 requests per minute
and 100 per rolling 24 hours per user, plus 120 requests per minute and 2,000 per
rolling 24 hours for the whole Shortcut service.
...
(`400`), more than 15 requests per minute or 100 per day (`429`), global
capacity exhaustion (`503`), unavailable AI (`503`), and
