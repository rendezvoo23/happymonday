...
export function AnalyticsCharts({ transactions }: AnalyticsChartsProps) {
    ...
    const currentMonthData = useMemo(() => {
        ...
        for (let d = 1; d <= daysInMonth; d++) {
            const dayStr = format(new Date(selectedDate.getFullYear(), selectedDate.getMonth(), d), 'yyyy-MM-dd');
    ...
    const prevMonthData = useMemo(() => {
        ...
        const dayData: MonthData[] = [];
        for (let d = 1; d <= daysInMonth; d++) {
            const dayStr = format(new Date(prevMonth.getFullYear(), prevMonth.getMonth(), d), 'yyyy-MM-dd');
