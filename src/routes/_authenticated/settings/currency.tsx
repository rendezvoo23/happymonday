          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="relative flex-1">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-6 h-6 text-gray-400 dark:text-gray-500 z-10" />
                <input
                  type="text"
                  value={searchQuery}
                  onFocus={handleSearchClick}
                  onBlur={(e) => setIsSearching(e.target.value.length > 0)}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="w-full pl-10 pr-4 py-[7px] backdrop-blur-sm rounded-full border-2 border-gray-200 dark:border-gray-700 bg-transparent text-gray-900 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:border-blue-400 dark:focus:border-blue-500 focus:outline-none transition-colors"
                />
              </div>
              {isSearching && (
                <LiquidButton
