export default {
  // Common
  common: {
    cancel: "Cancel",
    confirm: "Confirm",
    save: "Save",
    saveChanges: "Save Changes",
    delete: "Delete",
    edit: "Edit",
    add: "Add",
    close: "Close",
    back: "Back",
    next: "Next",
    loading: "Loading...",
    error: "Error",
    success: "Success",
    search: "Search",
    filter: "Filter",
    all: "All",
    none: "None",
    select: "Select",
    done: "Done",
    ok: "OK",
    yes: "Yes",
    no: "No",
  },

  // Navigation
  nav: {
    home: "Home",
    statistics: "Statistics",
    settings: "Settings",
    history: "History",
    ai: "AI Assistant",
  },

  // Settings Page
  settings: {
    title: "Settings",
    language: "Language",
    currency: "Currency",
    theme: "Theme",
    donate: "Donate",
    selectCurrency: "Select Currency",
    selectTheme: "Select Theme",
    selectLanguage: "Select Language",
    loadingSettings: "Loading settings...",
  },

  // Donate
  donate: {
    title: "Support Us",
    description: "Support our development with Telegram Stars",
    enterAmount: "Enter amount",
    send: "Send",
    processing: "...",
    coffee: "☕ Coffee",
    pizza: "🍕 Pizza",
    thankYou: "💝 Thank you",
    loveIt: "❤️ Love it",
    superFan: "🌟 Super fan",
    generous: "💎 Generous",
  },

  // Theme
  theme: {
    light: "Light",
    dark: "Dark",
    system: "System",
    automatic: "Automatic",
    lightDescription: "Always use light mode",
    darkDescription: "Always use dark mode",
    systemDescription: "Match device settings",
    automaticDescription: "Match system",
  },

  // History
  history: {
    sortByOccurredAt: "By date",
    sortByUpdatedAt: "By last updated",
    endOfList: "That's all",
  },

  // Transactions
  transactions: {
    add: "Add Transaction",
    edit: "Edit",
    delete: "Delete",
    expense: "Expense",
    income: "Income",
    amount: "Amount",
    amountPlaceholder: "0.00",
    category: "Category",
    subcategory: "Subcategory",
    subcategoryOptional: "Subcategory (Optional)",
    date: "Date",
    note: "Note",
    noteOptional: "Note (Optional)",
    notePlaceholder: "What was this for?",
    notes: "Notes",
    description: "Description",
    loadingTransactions: "Loading transactions...",
    loadMore: "Load more...",
    loadingCategories: "Loading categories...",
    loadingSubcategories: "Loading subcategories...",
    noTransactions: "No transactions yet",
    noCategories: "No categories available",
    addYourFirst: "Add your first transaction",
    recent: "Recent transactions",
    viewAll: "View all transactions",
    total: "Total",
    selectCategory: "Select Category",
    selectSubcategory: "Select Subcategory",
    enterAmount: "Enter amount",
    chooseSpendingCategory: "Choose a spending category",
    enterNote: "Enter note (optional)",
    confirmDelete: "Are you sure you want to delete this transaction?",
  },

  // Statistics
  statistics: {
    title: "Statistics",
    overview: "Overview",
    byCategory: "By Category",
    bySubcategory: "By Subcategory",
    trends: "Trends",
    totalExpenses: "Total Expenses",
    totalIncome: "Total Income",
    balance: "Balance",
    thisMonth: "This Month",
    lastMonth: "Last Month",
    thisYear: "This Year",
    noData: "No data available",
    day: "Day",
    week: "Week",
    month: "Month",
    year: "Year",
    deleteTransaction: "Delete Transaction",
    deleteConfirmation:
      "Are you sure you want to delete this transaction? This action cannot be undone.",
    cancel: "Cancel",
    delete: "Delete",
    deleting: "Deleting...",
    dailyAverage: "Daily Average",
    weeklyAverage: "Weekly Average",
    spending: "Spending",
    spendingToday: "Spending Today",
    spendingYesterday: "Spending Yesterday",
    spendingOnDate: "Spending {{date}}",
    totalToday: "Total Today",
    totalYesterday: "Total Yesterday",
    totalOnDate: "Total {{date}}",
    totalThisWeek: "Total This Week",
    totalLastWeek: "Total Last Week",
    totalWeekRange: "Total {{range}}",
    average: "avg",
    fromLastWeek: "from last week",
    fromLastMonth: "from last month",
    totalThisMonth: "Total This Month",
    totalLastMonth: "Total Last Month",
    totalForMonth: "Total {{month}}",
    totalThisYear: "Total This Year",
    totalLastYear: "Total Last Year",
    totalForYear: "Total {{year}}",
    monthlyAverage: "Monthly Average",
    fromLastYear: "from last year",
    weekShort: "W{{number}}",
    weekLong: "Week {{number}}",
  },

  // Categories
  categories: {
    food_drink: "Food & Drink",
    shopping: "Shopping",
    travel: "Travel",
    transportation: "Transport",
    services: "Services",
    entertainment: "Fun",
    health: "Health",
    salary: "Salary",
    investment: "Investment",
    other: "Other",
  },

  // Subcategories
  subcategories: {
    // Food
    groceries: "Groceries",
    restaurants: "Restaurants",
    cafes: "Cafes",
    fastFood: "Fast Food",
    delivery: "Delivery",

    // Transport
    publicTransport: "Public Transport",
    taxi: "Taxi",
    fuel: "Fuel",
    parking: "Parking",
    maintenance: "Maintenance",

    // Shopping
    clothing: "Clothing",
    electronics: "Electronics",
    furniture: "Furniture",
    gifts: "Gifts",
    accessories: "Accessories",

    // Entertainment
    movies: "Movies",
    games: "Games",
    sports: "Sports",
    hobbies: "Hobbies",
    subscriptions: "Subscriptions",

    // Health
    pharmacy: "Pharmacy",
    doctor: "Doctor",
    fitness: "Fitness",
    insurance: "Insurance",

    // Utilities
    electricity: "Electricity",
    water: "Water",
    gas: "Gas",
    internet: "Internet",
    phone: "Phone",

    // Housing
    rent: "Rent",
    mortgage: "Mortgage",
    repairs: "Repairs",
    cleaning: "Cleaning",

    // Travel
    flights: "Flights",
    hotels: "Hotels",
    visas: "Visas",
    tours: "Tours",
  },

  // Date & Time
  date: {
    today: "Today",
    yesterday: "Yesterday",
    tomorrow: "Tomorrow",
    thisWeek: "This Week",
    lastWeek: "Last Week",
    thisMonth: "This Month",
    lastMonth: "Last Month",
    thisYear: "This Year",
    lastYear: "Last Year",
    selectDate: "Select Date",
    selectMonth: "Select Month",
  },

  // Months
  months: {
    january: "January",
    february: "February",
    march: "March",
    april: "April",
    may: "May",
    june: "June",
    july: "July",
    august: "August",
    september: "September",
    october: "October",
    november: "November",
    december: "December",
  },

  // Short months
  monthsShort: {
    jan: "Jan",
    feb: "Feb",
    mar: "Mar",
    apr: "Apr",
    may: "May",
    jun: "Jun",
    jul: "Jul",
    aug: "Aug",
    sep: "Sep",
    oct: "Oct",
    nov: "Nov",
    dec: "Dec",
  },

  // Days of week
  days: {
    monday: "Monday",
    tuesday: "Tuesday",
    wednesday: "Wednesday",
    thursday: "Thursday",
    friday: "Friday",
    saturday: "Saturday",
    sunday: "Sunday",
  },

  // Short days
  daysShort: {
    mon: "Mon",
    tue: "Tue",
    wed: "Wed",
    thu: "Thu",
    fri: "Fri",
    sat: "Sat",
    sun: "Sun",
  },

  // Errors
  errors: {
    generic: "Something went wrong",
    network: "Network error",
    notFound: "Not found",
    unauthorized: "Unauthorized",
    validation: "Validation error",
    required: "This field is required",
    invalidAmount: "Invalid amount",
    invalidDate: "Invalid date",
  },

  // Success messages
  success: {
    transactionAdded: "Transaction added successfully",
    transactionUpdated: "Transaction updated successfully",
    transactionDeleted: "Transaction deleted successfully",
    settingsSaved: "Settings saved successfully",
  },

  // AI
  ai: {
    title: "AI Assistant",
    typeMessage: "Type your message...",
    send: "Send",
    analyzing: "Analyzing...",
    thinking: "Thinking...",
  },
};
