export default {
  // Common
  common: {
    cancel: "إلغاء",
    confirm: "تأكيد",
    save: "حفظ",
    saveChanges: "حفظ التغييرات",
    delete: "حذف",
    edit: "تعديل",
    add: "إضافة",
    close: "إغلاق",
    back: "رجوع",
    next: "التالي",
    loading: "جاري التحميل...",
    error: "خطأ",
    success: "نجح",
    search: "بحث",
    filter: "تصفية",
    all: "الكل",
    none: "لا شيء",
    select: "اختيار",
    done: "تم",
    ok: "موافق",
    yes: "نعم",
    no: "لا",
  },

  // Navigation
  nav: {
    home: "الرئيسية",
    statistics: "الإحصائيات",
    settings: "الإعدادات",
    history: "السجل",
    ai: "مساعد الذكاء الاصطناعي",
  },

  // Settings Page
  settings: {
    title: "الإعدادات",
    language: "اللغة",
    currency: "العملة",
    theme: "المظهر",
    donate: "تبرع",
    selectCurrency: "اختر العملة",
    selectTheme: "اختر المظهر",
    selectLanguage: "اختر اللغة",
    loadingSettings: "جاري تحميل الإعدادات...",
  },

  // Donate
  donate: {
    title: "ادعمنا",
    description: "ادعم تطويرنا بـ Telegram Stars",
    enterAmount: "أدخل المبلغ",
    send: "إرسال",
    processing: "...",
    coffee: "☕ قهوة",
    pizza: "🍕 بيتزا",
    thankYou: "💝 شكراً",
    loveIt: "❤️ أحبه",
    superFan: "🌟 معجب كبير",
    generous: "💎 كريم",
  },

  // Theme
  theme: {
    light: "فاتح",
    dark: "داكن",
    system: "النظام",
    automatic: "تلقائي",
    lightDescription: "استخدام الوضع الفاتح دائماً",
    darkDescription: "استخدام الوضع الداكن دائماً",
    systemDescription: "مطابقة إعدادات الجهاز",
    automaticDescription: "متابعة النظام",
  },

  // History
  history: {
    sortByOccurredAt: "حسب التاريخ",
    sortByUpdatedAt: "حسب آخر تحديث",
    endOfList: "انتهى",
  },

  // Transactions
  transactions: {
    add: "إضافة معاملة",
    edit: "تعديل",
    delete: "حذف",
    expense: "مصروف",
    income: "دخل",
    amount: "المبلغ",
    amountPlaceholder: "0.00",
    category: "الفئة",
    subcategory: "الفئة الفرعية",
    subcategoryOptional: "الفئة الفرعية (اختياري)",
    date: "التاريخ",
    note: "ملاحظة",
    noteOptional: "ملاحظة (اختياري)",
    notePlaceholder: "لماذا كان هذا؟",
    notes: "ملاحظات",
    description: "الوصف",
    loadingTransactions: "جاري تحميل المعاملات...",
    loadMore: "تحميل المزيد...",
    loadingCategories: "جاري تحميل الفئات...",
    loadingSubcategories: "جاري تحميل الفئات الفرعية...",
    noTransactions: "لا توجد معاملات بعد",
    noCategories: "لا توجد فئات متاحة",
    addYourFirst: "أضف معاملتك الأولى",
    recent: "المعاملات الأخيرة",
    viewAll: "عرض جميع المعاملات",
    total: "المجموع",
    selectCategory: "اختر الفئة",
    selectSubcategory: "اختر الفئة الفرعية",
    enterAmount: "أدخل المبلغ",
    chooseSpendingCategory: "اختر فئة الإنفاق",
    enterNote: "أدخل ملاحظة (اختياري)",
    confirmDelete: "هل أنت متأكد من حذف هذه المعاملة؟",
  },

  // Statistics
  statistics: {
    title: "الإحصائيات",
    overview: "نظرة عامة",
    byCategory: "حسب الفئة",
    bySubcategory: "حسب الفئة الفرعية",
    trends: "الاتجاهات",
    totalExpenses: "إجمالي المصروفات",
    totalIncome: "إجمالي الدخل",
    balance: "الرصيد",
    thisMonth: "هذا الشهر",
    lastMonth: "الشهر الماضي",
    thisYear: "هذا العام",
    noData: "لا توجد بيانات متاحة",
    day: "يوم",
    week: "أسبوع",
    month: "شهر",
    deleteTransaction: "حذف المعاملة",
    deleteConfirmation:
      "هل أنت متأكد أنك تريد حذف هذه المعاملة؟ لا يمكن التراجع عن هذا الإجراء.",
    cancel: "إلغاء",
    delete: "حذف",
    deleting: "جارٍ الحذف...",
    dailyAverage: "المتوسط اليومي",
    weeklyAverage: "المتوسط الأسبوعي",
    spending: "المصروف",
    spendingToday: "المصروف اليوم",
    spendingYesterday: "المصروف أمس",
    spendingOnDate: "المصروف {{date}}",
    totalToday: "المجموع اليوم",
    totalYesterday: "المجموع أمس",
    totalOnDate: "المجموع {{date}}",
    totalThisWeek: "إجمالي هذا الأسبوع",
    totalLastWeek: "إجمالي الأسبوع الماضي",
    totalWeekRange: "المجموع {{range}}",
    average: "متوسط",
    fromLastWeek: "من الأسبوع الماضي",
    fromLastMonth: "من الشهر الماضي",
    totalThisMonth: "إجمالي هذا الشهر",
    totalLastMonth: "إجمالي الشهر الماضي",
    totalForMonth: "المجموع {{month}}",
    weekShort: "أ{{number}}",
    weekLong: "الأسبوع {{number}}",
  },

  // Categories
  categories: {
    food_drink: "طعام وشراب",
    shopping: "تسوق",
    travel: "سفر",
    transportation: "مواصلات",
    services: "خدمات",
    entertainment: "ترفيه",
    health: "صحة",
    salary: "راتب",
    investment: "استثمار",
    other: "أخرى",
  },

  // Subcategories
  subcategories: {
    groceries: "بقالة",
    restaurants: "مطاعم",
    cafes: "مقاهي",
    fastFood: "وجبات سريعة",
    delivery: "توصيل",
    publicTransport: "مواصلات عامة",
    taxi: "تاكسي",
    fuel: "وقود",
    parking: "موقف سيارات",
    maintenance: "صيانة",
    clothing: "ملابس",
    electronics: "إلكترونيات",
    furniture: "أثاث",
    gifts: "هدايا",
    accessories: "إكسسوارات",
    movies: "أفلام",
    games: "ألعاب",
    sports: "رياضة",
    hobbies: "هوايات",
    subscriptions: "اشتراكات",
    pharmacy: "صيدلية",
    doctor: "طبيب",
    fitness: "لياقة",
    insurance: "تأمين",
    electricity: "كهرباء",
    water: "ماء",
    gas: "غاز",
    internet: "إنترنت",
    phone: "هاتف",
    rent: "إيجار",
    mortgage: "رهن عقاري",
    repairs: "إصلاحات",
    cleaning: "تنظيف",
    flights: "رحلات طيران",
    hotels: "فنادق",
    visas: "تأشيرات",
    tours: "جولات",
  },

  // Date & Time
  date: {
    today: "اليوم",
    yesterday: "أمس",
    tomorrow: "غداً",
    thisWeek: "هذا الأسبوع",
    lastWeek: "الأسبوع الماضي",
    thisMonth: "هذا الشهر",
    lastMonth: "الشهر الماضي",
    thisYear: "هذا العام",
    lastYear: "العام الماضي",
    selectDate: "اختر التاريخ",
    selectMonth: "اختر الشهر",
  },

  // Months
  months: {
    january: "يناير",
    february: "فبراير",
    march: "مارس",
    april: "أبريل",
    may: "مايو",
    june: "يونيو",
    july: "يوليو",
    august: "أغسطس",
    september: "سبتمبر",
    october: "أكتوبر",
    november: "نوفمبر",
    december: "ديسمبر",
  },

  // Short months
  monthsShort: {
    jan: "ينا",
    feb: "فبر",
    mar: "مار",
    apr: "أبر",
    may: "ماي",
    jun: "يون",
    jul: "يول",
    aug: "أغس",
    sep: "سبت",
    oct: "أكت",
    nov: "نوف",
    dec: "ديس",
  },

  // Days of week
  days: {
    monday: "الاثنين",
    tuesday: "الثلاثاء",
    wednesday: "الأربعاء",
    thursday: "الخميس",
    friday: "الجمعة",
    saturday: "السبت",
    sunday: "الأحد",
  },

  // Short days
  daysShort: {
    mon: "إثنين",
    tue: "ثلاثاء",
    wed: "أربعاء",
    thu: "خميس",
    fri: "جمعة",
    sat: "سبت",
    sun: "أحد",
  },

  // Errors
  errors: {
    generic: "حدث خطأ ما",
    network: "خطأ في الشبكة",
    notFound: "غير موجود",
    unauthorized: "غير مصرح",
    validation: "خطأ في التحقق",
    required: "هذا الحقل مطلوب",
    invalidAmount: "مبلغ غير صحيح",
    invalidDate: "تاريخ غير صحيح",
  },

  // Success messages
  success: {
    transactionAdded: "تمت إضافة المعاملة بنجاح",
    transactionUpdated: "تم تحديث المعاملة بنجاح",
    transactionDeleted: "تم حذف المعاملة بنجاح",
    settingsSaved: "تم حفظ الإعدادات بنجاح",
  },

  // AI
  ai: {
    title: "مساعد الذكاء الاصطناعي",
    typeMessage: "اكتب رسالتك...",
    send: "إرسال",
    analyzing: "جاري التحليل...",
    thinking: "جاري التفكير...",
  },
};
